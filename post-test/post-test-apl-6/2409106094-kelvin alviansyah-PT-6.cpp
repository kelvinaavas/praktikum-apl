...
// Fungsi menggunakan pointer (address-of & dereference)
void tambahAlbumPointer(Album* ptrAlbum, int* total) {
    if (*total >= MAKSIMUM_ALBUM) {
        cout << "Kapasitas album penuh.\n";
        return;
    }
    cout << "Masukkan nama album: ";
    cin.ignore();
    getline(cin, (ptrAlbum + *total)->nama);
    (*total)++;
    cout << "Album berhasil ditambahkan melalui pointer!\n";
}
...
//  Sorting huruf secara descending
void sortingAlbumDescending() {
    for (int i = 0; i < totalAlbum - 1; i++) {
        for (int j = 0; j < totalAlbum - i - 1; j++) {
            if (daftarAlbum[j].nama < daftarAlbum[j + 1].nama) {
                swap(daftarAlbum[j], daftarAlbum[j + 1]);
            }
        }
    }
    cout << "Daftar album berhasil diurutkan dari Z ke A!\n";
}

//  Sorting angka secara ascending
void sortingUserJumlahPesananAscending() {
    for (int i = 0; i < totalUser - 1; i++) {
        for (int j = 0; j < totalUser - i - 1; j++) {
            if (daftarUser[j].jumlahPesanan > daftarUser[j + 1].jumlahPesanan) {
                swap(daftarUser[j], daftarUser[j + 1]);
            }
        }
    }
    cout << "User berhasil diurutkan berdasarkan jumlah pesanan!\n";
}

// Sorting username ascending
void sortingUsernameAscending() {
    for (int i = 0; i < totalUser - 1; i++) {
        for (int j = 0; j < totalUser - i - 1; j++) {
    ...
    cout << "Username berhasil diurutkan dari A ke Z!\n";
}
...
void menuAdmin() {
    ...
    do {
        ...
        cout << "5. Urutkan Album Z-A\n6. Urutkan User berdasarkan Jumlah Pesanan\n7. Urutkan Username A-Z\n8. Kembali\n";
        cout << "Pilihan: ";
        cin >> pilihanAdmin;

        switch (pilihanAdmin) {
            case 1: tambahAlbumPointer(daftarAlbum, &totalAlbum); break;
            case 2: hapusAlbum(); break;
            case 3: tampilkanDaftarAlbum(); break;
            case 4: lihatUserDanPesanannya(); break;
            case 5: sortingAlbumDescending(); break;
            case 6: sortingUserJumlahPesananAscending(); break;
            case 7: sortingUsernameAscending(); break;
        }
    } while (pilihanAdmin != 8);
}
